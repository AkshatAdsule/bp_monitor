class Constants {
  static const String TABLE_BP_DATA = 'bpData';
  static const String COLUMN_ID = '_id';
  static const String COLUMN_SYSTOLIC = 'systolic';
  static const String COLUMN_DIASTOLIC = 'diastolic';
  static const String COLUMN_TIMESTAMP = 'timestamp';
  static const String DB_PATH = 'bpdata.db';

  static const int DIASTOLIC_MAX = 150;
  static const int DIASTOLIC_MIN = 60;

  static const int SYSTOLIC_MAX = 120;
  static const int SYSTOLIC_MIN = 60;
}
